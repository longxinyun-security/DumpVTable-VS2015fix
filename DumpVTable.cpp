    ...
    const std::wstring& TargetFilePath)
{
    ...
    auto libAttributeScope = make_unique_ptr(typeAttribute,[typeInfo](TYPEATTR* p) { typeInfo->ReleaseTypeAttr(p); });
    for (UINT i = 0; i < typeAttribute->cFuncs; ++i)
    {
        ...
        auto libAttributeScope = make_unique_ptr(funcDesc,[typeInfo](FUNCDESC* p) { typeInfo->ReleaseFuncDesc(p); });
...
HMODULE GetAssociatedModule(uintptr_t Address)
{
